...
<body>
    ...
    $getEvents = "SELECT * FROM events WHERE is_active = true and is_approved = true AND event_date >= CURRENT_DATE ORDER BY event_date LIMIT $limit OFFSET $offset";
    $resultGetEvents = pg_query($conn, $getEvents);

    // Запрос для подсчёта всех записей (без лимита и оффсета)
    $countQuery = "SELECT COUNT(*) FROM events WHERE is_active = true and is_approved = true AND event_date >= CURRENT_DATE";
